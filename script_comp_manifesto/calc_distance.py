...
def calc_distance(tf_idf_ret):
    ...
    for fromCategory, fromTokens in tf_idf_ret.items():
        for toCategory, toTokens in tf_idf_ret.items():
            ...
            v1 = [score for (term, score)
                  in sorted(fromTokens.term_scores.items())]
            v2 = [score for (term, score)
                  in sorted(toTokens.term_scores.items())]
            result[fromCategory][toCategory] \
